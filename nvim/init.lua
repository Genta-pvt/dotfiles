...
-- later(function()
--   add('https://github.com/vim-denops/denops-helloworld.vim')
-- end)
...
now(function()
  ...
  vim.keymap.set({ 'i', 'x' }, '<c-j>', '<Plug>(skkeleton-enable)', { desc = 'skkeleton-enable' })
end)
...
-- dcc
now(function()
  add('https://github.com/Shougo/ddc.vim')
  add('https://github.com/Shougo/ddc-filter-matcher_head')
  add('https://github.com/Shougo/ddc-filter-sorter_rank')
  add('https://github.com/Shougo/ddc-ui-native')
  ...
  vim.fn['skkeleton#config']({completionRankFile = '~/.skkeleton/rank.json'})
  vim.fn['ddc#enable']()
end)


-- 我が輩は猫である。名前はまだない。保管候補が出る事によって、送り仮名の入力が少し楽になった？
-- 例えば、教えるとか、考えるとかの分かち書きが雑に出来る様になったね。結構辞書の内容って多岐に渡るんだね。
-- カタカナ語は/の英数入力モードで入力してみると、かなりおもしろい。例えばディレクトリとか。ウィンドウとか。ユーザーも良いね。
